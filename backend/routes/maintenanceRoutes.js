...
router.post('/pay', async (req, res) => {
  try {
    ...
    // Check if whole year payment already exists
    const wholeYearPayment = await Maintenance.findOne({ email, month: 'Whole Year' });
    if (wholeYearPayment) {
      return res.status(400).send({ message: 'Whole year maintenance already paid for this resident' });
    }

    // If single month payment, check if it already exists
    if (month !== 'Whole Year') {
      const existingPayment = await Maintenance.findOne({ email, month });
      if (existingPayment) {
        return res.status(400).send({ message: `Maintenance already paid for this resident for ${month}` });
      }
    }
...
router.get('/status/:email', async (req, res) => {
  try {
    ...
    // Check for whole year payment
    if (paidMonths.includes('Whole Year')) {
      return res.json({ status: 'Whole year maintenance paid', paidMonths });
    }

    if (paidMonths.length === 12) {
      return res.json({ status: 'All month maintenance paid', paidMonths });
    }
    ...
    if (paidUpToDate) {
      return res.json({ status: 'Up to date', paidMonths });
    }

    const unpaidMonths = allMonths.slice(0, currentMonthIndex).filter(month => !paidMonths.includes(month));
    if (unpaidMonths.length > 0) {
      return res.json({ status: 'Maintenance baki hai', unpaidMonths, paidMonths });
    }

    res.json({ status: 'No unpaid maintenance before current month', paidMonths });
  } catch (error) {
    res.status(500).send({ error: 'Failed to check payment status' });
  }
});
